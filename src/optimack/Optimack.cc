...
void Optimack::send_optimistic_ack(struct subconn_info* conn, int cur_ack, int adjusted_rwnd){
    ...
    // log_info("S%u: sent ack %u, seq %u, tcp_win %u", conn->local_port, cur_ack, conn->next_seq_loc, cur_win_scaled);
    return;
}
...
int Optimack::send_ACK_adjusted_rwnd(struct subconn_info* conn, int cur_ack){ //std::chrono::time_point<std::chrono::system_clock>& last_send_ack, std::chrono::time_point<std::chrono::system_clock>& last_zero_window){
    ...
    log_info("S%u: sent ack %u, seq %u, win %d, win_end %u, tcp_win %u", conn->local_port, cur_ack, conn->opa_seq_start - conn->ini_seq_loc, cur_rwnd, rwnd/2 + cur_ack_rel, cur_win_scaled);
    if (diff <= 0)
        return -1;
    return 0;
...
Optimack::init()
{
    ...
    optim_ack_stop = nfq_stop = overrun_stop = cb_stop = -1;
    ...
    if(check_packet_lost_on_all_conns(recved_seq.getFirstEnd_withLock())){
        ...
        if(first_out_of_order){
            ...
            if(get_lost_range(&lost_all_range) >= 0){
                ...
                log_info("lost on all: request range[%u, %u]",lost_all_range.start, lost_all_range.end);
...
int
Optimack::establish_tcp_connection()
{
    int sockfd;
            ...
            case TH_ACK | TH_URG:
                {
                    ...
                    if (subconn_i == 0) {
                        ...
                        //Todo: cur_ack_rel < 
                        if(BACKUP_MODE){
                            subconn_info* subconn_backup = subconn_infos[backup_port];
                            ...
                            if(subconn_backup->recved_seq.size() > 0) {
                                ...
                                if(cur_ack_rel <= subconn_backup->recved_seq.getFirstEnd()){
                                    ...
                                    send_ACK_with_SACK(g_remote_ip, g_local_ip, g_remote_port, subconn_backup->local_port, sack_str, len, "", subconn_backup->ini_seq_rem + cur_ack_rel, subconn_backup->ini_seq_loc + subconn_backup->next_seq_loc, rwnd/win_scale);
            ...
            */
            ...
            case TH_ACK | TH_URG:
            {
                if (!payload_len) {
                    ...
                    if (subconn_i)
                        return 0;
                    send_optimistic_ack(subconn, seq_rel, get_ajusted_rwnd(seq_rel)); // Reply to Keep-Alive
