...
def parse_range_file(lines, out_file):
    avg_speed, recv_bytes, total_bytes = 0,0,1
    try:
        ...
        for line in lines[::-1]:
            if '83.4M' in line:
                ...
                break
        output = ','.join(map(str, [avg_speed, recv_bytes/total_bytes, recv_bytes, total_bytes]))
        print(output)
        return output
    except:
...
outfile_path = in_dir+'/range_output_'+os.path.basename(in_dir[:-1])+'.csv'
