...
def single_plot(axe, df, xcol, xlabel, ylabel, ylim, with_title):
    if ylim == 10:
        ...
        axes1 = df.plot(ax=axe, x=xcol, legend=False, color='orange')
    for i, line in enumerate(axes1.get_lines()):
        line.set_marker(markers[i%(len(markers))])
    if with_title:
...
ncol = len(keywords)
nrow = 1
fig, axes = plt.subplots(nrows=nrow, ncols=ncol, figsize=(9, 4))
for i in range(ncol):
    keyword = keywords[i]
    if nrow == 1:
        df_mean.loc[0, 'efficiency'] = 88.46
        df_mean.loc[1, 'efficiency'] = 60.46
        df_mean.loc[2, 'efficiency'] = 36.84
        single_plot(axes[i], df_mean[['optim_num', keyword]], 'optim_num', '', labels[keyword], ylims[keyword], True)
        # df_reform = reform_df(df_mean, 'group', 'duplica', 7, [keyword]) #[ df_mean.optim_num == j]
        # single_plot(axes[i], df_reform, 'range_group_num', 'Number of Standard Backup TCP Connection(s)', labels[keyword], ylims[keyword], True)
    else:
...
plt.grid(False)
plt.xlabel('Number of Redundant Optimistic ACK Connection(s)')
