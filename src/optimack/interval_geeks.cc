...
void IntervalListWithTime::clear(){
    Intervals.clear();
}

void IntervalListWithTime::clear_withLock(){
...
unsigned int IntervalListWithTime::total_bytes()
{
...
void IntervalListWithTime::insertNewInterval_withLock(unsigned int start, unsigned int end)
{
    pthread_mutex_lock(&mutex_intervals);
...
void IntervalListWithTime::insertNewInterval_withLock(Interval newInterval)
{
    pthread_mutex_lock(&mutex_intervals);
...
void IntervalListWithTime::insert_withLock(Interval newInterval)
{
    pthread_mutex_lock(&mutex_intervals);
    insert(newInterval);
    pthread_mutex_unlock(&mutex_intervals);
...
bool IntervalListWithTime::checkAndinsertNewInterval_withLock(unsigned int start, unsigned int end)
{
    pthread_mutex_lock(&mutex_intervals);
...
bool IntervalListWithTime::checkAndinsertNewInterval(unsigned int start, unsigned int end, int& order_flag)
{
    if(contains(start,end)){
        return false;
    }
...
bool IntervalListWithTime::checkAndinsertNewInterval_withLock(unsigned int start, unsigned int end, int& order_flag)
{
    bool ret;
    pthread_mutex_lock(&mutex_intervals);
...
unsigned int IntervalListWithTime::insertNewInterval_getLastEnd_withLock(unsigned int start, unsigned int end)
{
    unsigned lastend = 0;
    pthread_mutex_lock(&mutex_intervals);
    insertNewInterval(start, end);
...
void IntervalListWithTime::removeInterval_withLock(unsigned int start, unsigned int end)
{
    pthread_mutex_lock(&mutex_intervals);
...
void IntervalListWithTime::printIntervals_withLock()
{
    pthread_mutex_lock(&mutex_intervals);
    printIntervals();
    pthread_mutex_unlock(&mutex_intervals);
...
unsigned int IntervalListWithTime::getLastEnd_withLock()
{
    unsigned end = 0;
    pthread_mutex_lock(&mutex_intervals);
    if(!Intervals.empty())
...
unsigned int IntervalListWithTime::getFirstEnd()
{
    if (!Intervals.empty())
...
unsigned int IntervalListWithTime::getLastEnd()
{
    if (!Intervals.empty())
...
unsigned int IntervalListWithTime::getElem_withLock(unsigned int index, bool is_start)
{
    if(index < 0)
        return 0;
...
bool IntervalListWithTime::contains(unsigned int start, unsigned int end){
...
void IntervalListWithTime::insert(Interval newInterval)
{
    // Interval newInterval = Interval(start, end);
...
void IntervalListWithTime::insertNewInterval(Interval newInterval)
{
    insertNewInterval(newInterval.start, newInterval.end);
}
...
// Function to insert new interval and
// merge overlapping intervals
void IntervalListWithTime::insertNewInterval(unsigned int start, unsigned int end)
{
    Interval newInterval = Interval(start, end);
    std::vector<Interval> ans;
    int n = Intervals.size();
...
void IntervalListWithTime::removeInterval_updateTimer(unsigned int start, unsigned int end){
    std::vector<Interval> ans;
    int n = Intervals.size();
...
// Function to insert new interval and
// merge overlapping intervals
void IntervalListWithTime::removeInterval(unsigned int start, unsigned int end)
{
    std::vector<Interval> ans;
    int n = Intervals.size();
...
void IntervalListWithTime::substract(IntervalListWithTime* other){
...
void IntervalListWithTime::printIntervals(){
    for (size_t i = 0; i < Intervals.size(); i++)
        printf("[%u, %u], ", Intervals[i].start, Intervals[i].end);
    printf("\n");
}

std::string IntervalListWithTime::Intervals2str(){
    std::string result = "";
    char temp[100] = {0};
    for (size_t i = 0; i < Intervals.size(); i++){
        memset(temp, 0, 100);
